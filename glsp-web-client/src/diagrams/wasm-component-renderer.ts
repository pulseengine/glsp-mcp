...
export class WasmComponentRenderer {
  ...
  static renderWasmComponent(
    ...
    const label =
      ...
      "Component";
    const componentType =
      element.element_type || element.type || "wasm-component";
    const interfaces =
      (element.properties
        ?.interfaces as import("../types/wasm-component.js").ComponentInterface[]) ||
      [];
    ...
    let bgColor =
      componentType === "host-component" ? colors.host : colors.component;
    ...
    currentY = this.drawMetadataSection(
      ctx,
      {
        description,
        filePath,
    ...
    const importInterfaces = interfaces.filter(
      (i) =>
        i.interface_type === "import" ||
        (i as any).type === "import" ||
        (i as any).direction === "import",
    );
    const exportInterfaces = interfaces.filter(
      (i) =>
    ...
    const interfaceStartY = Math.max(
      currentY + 10,
      bounds.y + headerHeight + 20,
    );
    const availableHeight = bounds.y + bounds.height - interfaceStartY - 10;

    this.drawInterfaceConnectors(
      ctx,
      importInterfaces,
      bounds,
      "left",
  ...
  private static drawInterfaceConnectors(
    ...
    const spacing = Math.min(
      this.INTERFACE_SPACING,
      availableHeight / Math.max(interfaces.length, 1),
    );
  ...
  private static drawMetadataSection(
    ...
    if (metadata.dependencies.length > 0) {
      ctx.fillStyle = "rgba(0, 0, 0, 0.6)";
      ctx.font =
        '8px -apple-system, BlinkMacSystemFont, "Segoe UI", Roboto, sans-serif';
      ctx.fillText(
