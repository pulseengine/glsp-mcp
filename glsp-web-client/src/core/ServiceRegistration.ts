...
export function registerServices(canvas: HTMLCanvasElement): void {
  ...
  serviceContainer.register<McpService>({
    ...
    lifecycle: {
      ...
      onReady: (service) => {
        console.log(
          "MCP service ready, setting up connection monitoring and execution streaming...",
        );
  ...
  serviceContainer.register<ViewModeManager>({
    ...
    factory: async () => {
      const diagramService =
        await serviceContainer.get<DiagramService>("diagramService");
      const renderer =
        await serviceContainer.get<CanvasRenderer>("canvasRenderer");
      const viewModeManager = new ViewModeManager(diagramService, renderer);

      // Register the WASM view transformer
      viewModeManager.registerTransformer(
        "wasm-component",
        new WasmViewTransformer(),
      );
  ...
  serviceContainer.register<WasmRuntimeManager>({
    ...
    lifecycle: {
      onReady: async (wasmRuntimeManager) => {
        console.log(
          "WASM Runtime Manager ready, initializing enhanced components...",
        );
  ...
  serviceContainer.register<InteractionManager>({
    ...
    lifecycle: {
      onReady: async (interactionManager) => {
        console.log(
          "Interaction Manager ready, setting up dependencies and event handlers...",
        );

        // Get dependent services
        const wasmRuntimeManager =
          await serviceContainer.get<WasmRuntimeManager>("wasmRuntimeManager");
        const uiManager = await serviceContainer.get<UIManager>("uiManager");
        const viewModeManager =
          await serviceContainer.get<ViewModeManager>("viewModeManager");
