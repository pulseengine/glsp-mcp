...
function convertToWitInterface(
  ...
  const witFunctions: WitFunction[] = (componentInterface.functions || []).map(
    (func) => ({
      name: func.name,
      params:
        (func.parameters as any[])?.map((param: any) => ({
          name: param.name || "param",
          param_type: param.type || "unknown",
        })) || [],
      results: func.return_type
        ? [{ name: "result", param_type: func.return_type }]
        : [],
    }),
  );
...
export class CanvasRenderer {
  ...
  private graphicsIntegration: GraphicsComponentIntegration | null = null;
  private graphicsBridge: GraphicsBridge | null = null;
  ...
  constructor(canvas: HTMLCanvasElement, options: RenderOptions = {}) {
    ...
    // Initialize graphics integration
    this.initializeGraphicsIntegration();

    // Setup graphics debugging in development
    this.setupGraphicsDebugging();
  ...
  private setupEventListeners(): void {
    ...
    this.showInterfaceNames =
      localStorage.getItem("showInterfaceNames") === "true";
  }
  ...
  private getMousePosition(event: MouseEvent): Position {
    ...
    const worldPos = {
      x:
        (event.clientX - rect.left - this.options.offset.x) /
        this.options.scale,
      y:
        (event.clientY - rect.top - this.options.offset.y) / this.options.scale,
    };
    return worldPos;
  }
  ...
  private isPointOnEdge(point: Position, edge: Edge): boolean {
    ...
    for (const segment of segments) {
      if (
        this.distanceToLineSegment(point, segment.start, segment.end) <=
        scaledTolerance
      ) {
        return true;
      }
  ...
  protected handleClick(event: MouseEvent): void {
    ...
    // Try graphics components first
    if (this.handleGraphicsComponentMouseEvent(event, "click")) {
      return;
    }
    ...
    console.log(
      "handleClick: Mode detected as:",
      mode,
      "InteractionMode.CreateInterfaceLink:",
      InteractionMode.CreateInterfaceLink,
    );
    console.log(
      "handleClick: Mode comparison result:",
      mode === InteractionMode.CreateInterfaceLink,
    );
    ...
    if (mode === InteractionMode.CreateInterfaceLink) {
      ...
      if (interfaceConnector) {
        console.log(
          `INTERFACE CLICK SUCCESS: ${interfaceConnector.interface.name}`,
        );
        this.emit({
          ...
          interfaceInfo: {
            ...
            interfaceType:
              interfaceConnector.side === "left" ? "import" : "export",
        ...
        console.log(
          "handleClick: No interface connector found, canceling linking",
        );
      ...
      case InteractionMode.CreateEdge: {
        ...
        if (interfaceConnector) {
          ...
          if (!this.edgeCreationSource) {
            ...
            interfaceConnector.element.id !== this.edgeCreationSource.id
          ) {
          ...
          const elementTypeForEdge =
            element?.type || element?.element_type || "";
          if (
            element &&
            (elementTypeForEdge === "task" ||
              elementTypeForEdge.includes("event") ||
              elementTypeForEdge === "gateway" ||
              elementTypeForEdge === "wasm-component")
          ) {
            if (!this.edgeCreationSource) {
              // Start edge creation
  ...
  protected handleDoubleClick(event: MouseEvent): void {
    ...
    if (element) {
      console.log(
        "handleDoubleClick: Element found:",
        element.id,
        element.type,
      );

      // Check if this is a WASM component that can be executed
      const isWasmComponent =
        element.type === "wasm-component" ||
        element.element_type === "wasm-component" ||
        element.properties?.componentType === "wasm-component";

      if (isWasmComponent) {
        console.log(
          "handleDoubleClick: WASM component detected, emitting double-click event",
        );
        this.emit({
          type: "double-click",
          position,
  ...
  private handleMouseMove(event: MouseEvent): void {
    // Try graphics components first
    if (this.handleGraphicsComponentMouseEvent(event, "move")) {
      return;
    }
    ...
    if (this.isDragging && this.dragStart) {
      ...
      if (interfaceConnector) {
        ...
        const currentHoverKey = this.hoveredConnector
          ? `${this.hoveredConnector.element.id}-${this.hoveredConnector.interface.name}-${this.hoveredConnector.side}`
          : null;
      ...
      if (
        ...
        element &&
        this.selectionManager.isSelected(element.id)
      ) {
  ...
  private handleMouseDown(event: MouseEvent): void {
    // Try graphics components first
    if (this.handleGraphicsComponentMouseEvent(event, "down")) {
      return;
    }
    ...
    if (
      event.button === 1 ||
      (event.button === 0 && mode === InteractionMode.Pan)
    ) {
      this.isPanning = true;
      this.panStart = { x: event.clientX, y: event.clientY };
      this.canvas.style.cursor = "move";
  ...
  private handleMouseUp(event: MouseEvent): void {
    // Try graphics components first
    if (this.handleGraphicsComponentMouseEvent(event, "up")) {
      return;
    }
  ...
  private handleWheel(event: WheelEvent): void {
    ...
    if (event.ctrlKey || event.metaKey) {
      ...
      const newScale = Math.max(
        this.minScale,
        Math.min(this.maxScale, this.options.scale * scaleFactor),
      );
  ...
  private showWitContextMenu(event: MouseEvent, element: ModelElement): void {
    ...
    viewWitOption.style.cssText = `
        ...
        `;
    viewWitOption.textContent = isInterface
      ? "View WIT Interface"
      : "View WIT Component";
  ...
  private gatherComponentInterfaces(component: ModelElement): any[] {
    ...
    Object.values(this.currentDiagram.elements).forEach((element) => {
      ...
      if (elementType === "uml-interface") {
        ...
        if (
          ...
          parentComponent === originalComponentId
        ) {
          interfaces.push({
            name: element.label?.toString() || "interface",
            interface_type: element.properties?.interfaceType || "import",
    ...
    if (interfaces.length === 0 && component.properties?.originalInterfaces) {
      return Array.isArray(component.properties.originalInterfaces)
        ? component.properties.originalInterfaces
        : [];
    }
  ...
  setDiagram(diagram: DiagramModel): void {
    ...
    window.dispatchEvent(
      new CustomEvent("diagram-edge-type-loaded", {
    ...
    console.log(
      "CanvasRenderer: Diagram loaded with edge type:",
      savedEdgeType,
    );
  }
  ...
  zoom(scaleFactor: number, center?: Position): void {
    ...
    if (center) {
      ...
      this.options.offset.x =
        center.x -
        (center.x - this.options.offset.x) * (newScale / this.options.scale);
      this.options.offset.y =
        center.y -
        (center.y - this.options.offset.y) * (newScale / this.options.scale);
    } else {
      ...
      this.options.offset.x =
        centerX -
        (centerX - this.options.offset.x) * (newScale / this.options.scale);
      this.options.offset.y =
        centerY -
        (centerY - this.options.offset.y) * (newScale / this.options.scale);
    }
  ...
  fitToContent(): void {
    ...
    const elements = Object.values(this.currentDiagram.elements).filter(
      (e) => e.bounds,
    );
  ...
  private constrainPan(): void {
    ...
    if (contentWidth > canvasWidth) {
      ...
      this.options.offset.x = Math.max(
        minOffsetX,
        Math.min(maxOffsetX, this.options.offset.x),
      );
    }
    ...
    if (contentHeight > canvasHeight) {
      ...
      this.options.offset.y = Math.max(
        minOffsetY,
        Math.min(maxOffsetY, this.options.offset.y),
      );
    }
  }
  ...
  private drawGrid(): void {
    const gridSize = 20;
    const startX =
      Math.floor(-this.options.offset.x / this.options.scale / gridSize) *
      gridSize;
    const startY =
      Math.floor(-this.options.offset.y / this.options.scale / gridSize) *
      gridSize;
    const endX =
      startX + this.canvas.clientWidth / this.options.scale + gridSize;
    const endY =
      startY + this.canvas.clientHeight / this.options.scale + gridSize;
  ...
  private drawNode(
    ...
    if (this.isWasmComponentType(nodeType)) {
      ...
      if (this.currentViewMode === "uml-interface") {
        ...
        const optimalSize = UMLComponentRenderer.calculateOptimalSize(
          node,
          umlContext.style,
          umlContext,
        );
  ...
  private drawEdge(edge: Edge): void {
    ...
    const sourceId = String(
      edge.source || edge.sourceId || edge.properties?.sourceId || "",
    );
    const targetId = String(
      edge.target || edge.targetId || edge.properties?.targetId || "",
    );
    ...
    this.drawEdgeByType(
      ...
      targetCenter,
      edge.routingPoints,
      this.edgeCreationType || "straight",
    );
    ...
    // Draw edge label
    const edgeLabel =
      edge.label?.toString() || edge.properties?.label?.toString();
    if (edgeLabel) {
      const midPoint = {
  ...
  private drawCurvedEdge(
    ...
    if (routingPoints && routingPoints.length > 0) {
      ...
      const controlY =
        sourceCenter.y - Math.abs(targetCenter.x - sourceCenter.x) * 0.2; // Curve upward
      this.ctx.quadraticCurveTo(
        controlX,
        controlY,
        targetCenter.x,
  ...
  private drawEdgePreview(): void {
    ...
    const sourceCenter = {
      x:
        this.edgeCreationSource.bounds.x +
        this.edgeCreationSource.bounds.width / 2,
      y:
        this.edgeCreationSource.bounds.y +
        this.edgeCreationSource.bounds.height / 2,
    };
    ...
    if (targetConnector && this.sourceInterfaceInfo) {
      isCompatible = this.isInterfaceCompatible(
        this.sourceInterfaceInfo.interface,
        targetConnector.interface,
      );
    }

    // Set edge color based on compatibility
    this.ctx.strokeStyle = isCompatible
      ? this.options.selectedColor
      : "#F44336"; // Red for incompatible
    this.ctx.lineWidth = 2;
    this.ctx.setLineDash([5, 5]);
    ...
    if (targetConnector) {
      ...
      this.ctx.beginPath();
      this.ctx.arc(
        this.edgePreviewTarget.x,
        this.edgePreviewTarget.y,
        8,
        0,
        2 * Math.PI,
      );
      this.ctx.fillStyle = isCompatible
        ? "rgba(76, 175, 80, 0.3)"
        : "rgba(244, 67, 54, 0.3)";
      this.ctx.fill();
      this.ctx.stroke();
  ...
  private saveEdgeTypePreference(edgeType: string): void {
    ...
    if (!this.currentDiagram.metadata) {
      this.currentDiagram.metadata = {
        ...
        type:
          ...
          this.currentDiagram.diagram_type ||
          "unknown",
    ...
    console.log(
      "CanvasRenderer: Saved edge type preference:",
      edgeType,
      "to diagram:",
  ...
  private loadEdgeTypePreference(): string {
    const savedEdgeType =
      this.currentDiagram?.metadata?.preferences?.edgeCreationType;
    if (savedEdgeType) {
      console.log(
        "CanvasRenderer: Loaded edge type preference:",
        savedEdgeType,
        "from diagram:",
        this.currentDiagram?.id,
      );
  ...
  private updateInterfaceHighlights(): void {
    ...
    for (const element of Object.values(this.currentDiagram.elements)) {
      ...
      interfaces.forEach((iface: ComponentInterface) => {
        if (
          this.isInterfaceCompatible(this.sourceInterfaceInfo!.interface, iface)
        ) {
  ...
  private _findInterfaceConnector(position: Position):
    ...
    for (const element of Object.values(this.currentDiagram.elements)) {
      ...
      const inputs = interfaces.filter(
        (i: { interface_type?: string; type?: string; direction?: string }) =>
          i.interface_type === "import" ||
          i.type === "import" ||
          i.direction === "input",
      );
      const outputs = interfaces.filter(
        (i: { interface_type?: string; type?: string; direction?: string }) =>
      ...
      const result = this.checkInterfacePorts(
        element,
        inputs,
        "left",
        position,
      ...
      const outputResult = this.checkInterfacePorts(
        element,
        outputs,
        "right",
        position,
  ...
  async getMissingComponents(): Promise<
    Array<{
      path: string;
  ...
  > {
    ...
    try {
      const missingResource = await this.mcpClient.readResource(
        "wasm://components/missing",
      );
  ...
  async handleMissingComponent(
    ...
    try {
      ...
      if (status.path) {
        console.log(
          `To restore component "${componentName}", place the WASM file at: ${status.path}`,
        );
        return true;
      }
    } catch (error) {
  ...
  private showInterfaceTooltip(
    ...
    this.interfaceTooltip.innerHTML = `
            ...
            <div class="tooltip-type">${
              connector.side === "left" ? "Import" : "Export"
            } Interface</div>
            ${
              (connector.interface as any).functions &&
              (connector.interface as any).functions.length > 0
                ? `<div class="tooltip-functions">${
                    (connector.interface as any).functions.length
                  } function(s)</div>`
                : ""
            }
        `;
  ...
  private drawEmptyState(): void {
    ...
    // Draw message
    this.ctx.font =
      '18px -apple-system, BlinkMacSystemFont, "Segoe UI", Roboto, Arial, sans-serif';
    this.ctx.fillStyle = this.options.textColor;
    this.ctx.textAlign = "center";
    this.ctx.textBaseline = "middle";
    ...
    this.ctx.font =
      '48px -apple-system, BlinkMacSystemFont, "Segoe UI", Roboto, Arial, sans-serif';
  ...
  private ensureElementBounds(element: any): void {
    if (
      !element.bounds &&
      this.isWitInterfaceType(this.getElementType(element))
    ) {
      // Get default size from diagram type config
      const config = getDiagramTypeConfig("wit-interface");
      const nodeType = config?.nodeTypes.find(
        (n) => n.type === this.getElementType(element),
      );
  ...
  private isWitViewMode(): boolean {
    return (
      this.currentViewMode === "wit-interface" ||
      this.currentViewMode === "wit-dependencies"
    );
  }
  ...
  private shouldRenderElement(
    ...
    // Component filtering
    if (
      renderingHints.showComponents === false &&
    ...
    // Interface filtering
    if (
      renderingHints.showInterfaces === false &&
      this.isInterfaceElement(element)
    ) {
      return false;
    }
    ...
    if (
      renderingHints.showPackages === false &&
      elementType === "wit-package"
    ) {
      return false;
    }
    ...
    if (
      renderingHints.showFunctions === false &&
      elementType === "wit-function"
    ) {
      return false;
    }
    ...
    for (const element of Object.values(this.currentDiagram.elements)) {
      ...
      interfaces.forEach((iface: ComponentInterface, index: number) => {
        if (
          ...
          const connectorPos = this.calculateConnectorPosition(
            element,
            index,
            side,
          );
   ...
   */
  private isInterfaceCompatible(
    sourceInterface: ComponentInterface,
    targetInterface: ComponentInterface,
  ): boolean {
    // Basic compatibility check: export can connect to import and vice versa
    const sourceType =
      sourceInterface.interface_type ||
      sourceInterface.type ||
      sourceInterface.direction;
    const targetType =
      targetInterface.interface_type ||
      targetInterface.type ||
      targetInterface.direction;

    // Export/Output can connect to Import/Input
    if (
      ((sourceType as string) === "export" ||
        (sourceType as string) === "output") &&
      ((targetType as string) === "import" ||
        (targetType as string) === "input")
    ) {
      return true;
    }
    ...
    if (
      ((sourceType as string) === "import" ||
        (sourceType as string) === "input") &&
      ((targetType as string) === "export" ||
        (targetType as string) === "output")
    ) {
      return true;
    }
  ...
  private getInterfaceSide(iface: ComponentInterface): "left" | "right" {
    ...
    return (type as string) === "import" || (type as string) === "input"
      ? "left"
      : "right";
  }
   ...
   */
  private async initializeGraphicsIntegration(): Promise<void> {
    try {
      // Skip graphics integration if no MCP client available
      if (!this.mcpClient) {
        console.log(
          "CanvasRenderer: Skipping graphics integration - no MCP client available",
        );
        return;
      }

      // Create graphics bridge with canvas and MCP service
      this.graphicsBridge = new GraphicsBridge(this.canvas, this.mcpClient);

      // Create graphics integration
      this.graphicsIntegration = new GraphicsComponentIntegration(
        this,
        this.graphicsBridge,
        {
      ...
      await this.graphicsIntegration.initialize();

      console.log("CanvasRenderer: Graphics component integration initialized");
    } catch (error) {
      console.warn(
        "CanvasRenderer: Failed to initialize graphics integration:",
        error,
      );
    }
  }
  ...
  /**
   * Check if a node is a graphics component
   */
  public isGraphicsComponentNode(node: Node): node is GraphicsComponentNode {
    return (node as GraphicsComponentNode).isGraphicsComponent === true;
  }

  /**
   * Render graphics components within the main canvas
   */
  protected renderGraphicsComponents(): void {
    if (!this.graphicsIntegration || !this.currentDiagram) return;

    Object.values(this.currentDiagram.elements).forEach((element) => {
      if (element.type === "node") {
        const node = element as Node;
        if (this.isGraphicsComponentNode(node)) {
          this.renderGraphicsComponentNode(node);
        }
      }
    });
  }
  ...
  /**
   * Render a single graphics component node
   */
  private renderGraphicsComponentNode(node: GraphicsComponentNode): void {
    if (!node.graphicsCanvas) return;

    const { x, y } = node.position || { x: 0, y: 0 };
    const { width, height } = node.size || { width: 0, height: 0 };

    // Draw the graphics canvas onto the main canvas
    try {
      this.ctx.save();

      // Draw background for the graphics component
      this.ctx.fillStyle = "var(--bg-secondary, #151B2C)";
      this.ctx.fillRect(x, y, width, height);

      // Draw border
      this.ctx.strokeStyle = "var(--border-color, #2A3441)";
      this.ctx.lineWidth = 1;
      this.ctx.strokeRect(x, y, width, height);

      // Draw graphics component canvas
      const canvasX = x + 20; // Offset for padding
      const canvasY = y + 30; // Offset for title area
      ...
      this.ctx.drawImage(
        ...
        0,
        0,
        node.graphicsCanvas.width,
        node.graphicsCanvas.height,
        canvasX,
        canvasY,
        canvasWidth,
        canvasHeight,
      ...
      // Draw title
      this.ctx.fillStyle = "var(--text-primary, #E6EDF3)";
      this.ctx.font =
        '14px -apple-system, BlinkMacSystemFont, "Segoe UI", sans-serif';
      this.ctx.textAlign = "center";
      this.ctx.fillText(
        node.label || "Graphics Component",
        x + width / 2,
        y + 20,
      );
      ...
      console.error("Error rendering graphics component node:", error);
      ...
      this.ctx.textAlign = "center";
      this.ctx.fillText("Graphics Error", x + width / 2, y + height / 2);
      this.ctx.restore();
    }
  }
   ...
   */
  protected handleGraphicsComponentMouseEvent(
    event: MouseEvent,
    type: "click" | "move" | "down" | "up",
  ): boolean {
    if (!this.graphicsIntegration || !this.currentDiagram) return false;

    const rect = this.canvas.getBoundingClientRect();
    const x = event.clientX - rect.left;
    const y = event.clientY - rect.top;

    // Find graphics component under mouse
    for (const element of Object.values(this.currentDiagram.elements)) {
      if (element.type === "node") {
        const node = element as Node;
        if (this.isGraphicsComponentNode(node)) {
          ...
          if (
            ...
            // Handle the event with the graphics component
            const handled = this.graphicsIntegration.handleMouseEvent(
              node,
              event,
              type,
            );
  ...
  /**
   * Get graphics integration instance
   */
  public getGraphicsIntegration(): GraphicsComponentIntegration | null {
    return this.graphicsIntegration;
  }

  /**
   * Add a graphics component to the diagram
   */
  public async addGraphicsComponent(
    componentName: string,
    position: { x: number; y: number },
  ): Promise<string | null> {
    if (!this.graphicsIntegration || !this.currentDiagram) return null;

    try {
      // Create a mock WASM component for the graphics component
      const mockComponent = {
        name: componentName,
        path: `graphics:${componentName}`,
        description: `Graphics component: ${componentName}`,
        exports: [{ name: "render", type: "func" }],
        imports: [],
        metadata: {
          graphics: true,
          type: "graphics",
      ...
      // Create graphics component node
      const node = await this.graphicsIntegration.createGraphicsComponentNode(
        mockComponent as any,
        position,
      );

      // Add to diagram
      this.currentDiagram.elements[node.id] = node;

      // Render update
      this.render();

      console.log(
        `CanvasRenderer: Added graphics component '${componentName}' with ID '${node.id}'`,
      );
      return node.id;
    } catch (error) {
      console.error("CanvasRenderer: Failed to add graphics component:", error);
      return null;
    }
  }

  /**
   * Remove a graphics component from the diagram
   */
  public removeGraphicsComponent(nodeId: string): void {
    if (!this.graphicsIntegration || !this.currentDiagram) return;

    const element = this.currentDiagram.elements[nodeId];
    if (element && this.isGraphicsComponentNode(element as Node)) {
      // Remove from graphics integration
      this.graphicsIntegration.removeGraphicsComponent(nodeId);

      // Remove from diagram
      delete this.currentDiagram.elements[nodeId];
      ...
      console.log(`CanvasRenderer: Removed graphics component '${nodeId}'`);
    }
  }
   ...
   */
  public getAvailableGraphicsComponents(): any[] {
    return this.graphicsIntegration?.getAvailableGraphicsComponents() || [];
  }

  /**
   * Update graphics component properties
   */
  public async updateGraphicsComponentProperties(
    nodeId: string,
    properties: Record<string, any>,
  ): Promise<void> {
    if (this.graphicsIntegration) {
      await this.graphicsIntegration.updateComponentProperties(
        nodeId,
        properties,
      );
      this.render();
  ...
  /**
   * Enhanced destroy method to clean up graphics integration
   */
  public destroy(): void {
    if (this.graphicsIntegration) {
      this.graphicsIntegration.destroy();
      this.graphicsIntegration = null;
    }

    this.graphicsBridge = null;

    // Cleanup handled by individual components
  }

  // Make graphics integration methods available globally for debugging
  private setupGraphicsDebugging(): void {
    if (
      typeof window !== "undefined" &&
      process.env.NODE_ENV === "development"
    ) {
      (window as any).graphicsRenderer = {
        addComponent: (name: string, x: number = 100, y: number = 100) =>
          this.addGraphicsComponent(name, { x, y }),
        removeComponent: (id: string) => this.removeGraphicsComponent(id),
