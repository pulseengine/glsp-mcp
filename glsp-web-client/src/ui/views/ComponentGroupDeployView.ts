...
export class ComponentGroupDeployView {
  ...
  private createPreviewTab(
    ...
    tab.style.cssText = `
            padding: 12px 20px;
            background: ${
              active ? "var(--bg-secondary, #151B2C)" : "transparent"
            };
            border: none;
            border-bottom: 2px solid ${
              active ? "var(--accent-wasm, #654FF0)" : "transparent"
            };
            color: ${
              active
...
wac_compose(
    ...
    components = {
${this.groupData.components
  .map(
    (c) =>
      `        "${c.name.toLowerCase().replace(/[^a-z0-9]/g, "_")}": ":${c.name
        .toLowerCase()
        .replace(/[^a-z0-9]/g, "_")}_${this.config.profile}",`,
  )
  .join("\n")}
    },
...
${
  this.config.validation.validateComponents
    ? this.generateValidationTargets()
    : ""
}

# Individual component targets (assuming they exist)
${this.groupData.components
  .map((c) => this.generateComponentTarget(c.name, this.config.profile))
  .join("\n\n")}`;
  }
...
rust_wasm_component(
    ...
    profile = "${profile}",
    ${
      this.config.optimizations.stripDebugInfo && profile === "release"
  ...
  private generateWacFile(): string {
    return `# Generated WAC configuration for component group: ${
      this.groupData.name
    }
# Generated by GLSP Component Grouping System
...
${this.groupData.components
  .map((c) => this.generateComponentDeclaration(c.name))
  .join("\n")}

# Component connections
${this.groupData.internalConnections
  .map((conn) => this.generateConnectionDeclaration(conn))
  .join("\n")}

# External interface exports
${this.groupData.externalInterfaces
  .map((iface) => this.generateExportDeclaration(iface))
  .join("\n")}`;
  }
