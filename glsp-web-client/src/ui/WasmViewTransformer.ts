...
import {
  ViewTransformer,
  ViewTransformationResult,
} from "./ViewModeManager.js";
import {
  WasmComponentData,
  ComponentInterface,
} from "../types/wasm-component.js";
...
export class WasmViewTransformer implements ViewTransformer {
  ...
  public canTransform(
    ...
    if (
      !supportedViews.includes(fromView) ||
      !supportedViews.includes(toView)
    ) {
      return false;
    }
   ...
   */
  private transformToComponentView(
    diagram: DiagramModel,
  ): ViewTransformationResult {
  ...
  private transformToUMLView(diagram: DiagramModel): ViewTransformationResult {
    ...
    wasmComponents.forEach((component, componentIndex) => {
      ...
      const interfaceLayoutForComponent =
        layout.interfaceLayout[componentIndex];
      ...
      interfaceLayoutForComponent.interfaces.forEach((interfaceLayout) => {
        // Find the corresponding interface data
        const interfaceData = component.interfaces.find(
          (iface) => iface.name === interfaceLayout.name,
        );
  ...
  private calculateUMLAutoLayout(components: WasmComponentData[]): {
    ...
    const calculateInterfaceSize = (iface: any) => {
      ...
      functions.forEach((func: any) => {
        const funcText = `${func.name}(${(func.params || [])
          .map((p: any) => `${p.name}: ${p.param_type}`)
          .join(", ")})`;
      ...
      const calculatedHeight =
        baseHeight + functions.length * functionHeight + 20; // Extra padding
    ...
    const componentData = components.map((component, index) => {
      const importInterfaces = component.interfaces.filter(
        (iface) => iface.type === "import",
      );
      const exportInterfaces = component.interfaces.filter(
        (iface) => iface.type === "export",
      );
      ...
      const importInterfaceSizes = importInterfaces.map((iface) =>
        calculateInterfaceSize(iface),
      );
      const exportInterfaceSizes = exportInterfaces.map((iface) =>
        calculateInterfaceSize(iface),
      );

      // Calculate required space for interfaces
      const importHeight =
        importInterfaceSizes.length > 0
          ? importInterfaceSizes.reduce(
              (total, size, index) =>
                total + size.height + (index > 0 ? interfaceSpacing : 0),
              0,
            )
          : interfaceHeight;
      const exportHeight =
        exportInterfaceSizes.length > 0
          ? exportInterfaceSizes.reduce(
              (total, size, index) =>
                total + size.height + (index > 0 ? interfaceSpacing : 0),
              0,
            )
    ...
    const maxInterfaceWidth = Math.max(
      ...componentData.map((cd) => cd.interfaceWidth),
    );
    const totalWidth =
      interfaceMargin +
      maxInterfaceWidth +
    ...
    const componentStartX =
      interfaceMargin + maxInterfaceWidth + componentSpacing;
    ...
    componentData.forEach((data, componentIndex) => {
      ...
      const importStartY =
        currentY +
        (requiredHeight -
          (importInterfaces.length * (interfaceHeight + interfaceSpacing) -
            interfaceSpacing)) /
          2;
      ...
      const exportStartY =
        currentY +
        (requiredHeight -
          (exportInterfaces.length * (interfaceHeight + interfaceSpacing) -
            interfaceSpacing)) /
          2;
  ...
  private calculateLayoutParameters(components: WasmComponentData[]): {
    ...
    const maxFunctionsPerInterface = Math.max(
      ...components.flatMap((c) =>
        c.interfaces.map((i) => i.functions?.length || 0),
      ),
    );
   ...
   */
  private transformToInterfaceView(
    diagram: DiagramModel,
  ): ViewTransformationResult {
    const elements = Object.values(diagram.elements);
    const wasmComponents = this.extractWasmComponents(elements);
    ...
    wasmComponents.forEach((component) => {
      ...
      if (importInterfaces.length > 0) {
        // const importGroupY = interfaceY; // Unused

        importInterfaces.forEach((iface) => {
          ...
          witElements.push(
            ...functionElements.nodes,
            ...functionElements.edges,
          );
          ...
          interfaceY +=
            layout.interfaceVerticalSpacing +
            (iface.functions?.length || 0) * layout.functionVerticalSpacing;
        });
      }
      ...
      if (exportInterfaces.length > 0) {
        exportInterfaces.forEach((iface) => {
          const interfaceNode = this.createInterfaceNode(
            nodeIdCounter++,
            iface,
            component.id,
   ...
   */
  private transformToDependencyView(
    diagram: DiagramModel,
  ): ViewTransformationResult {
    const elements = Object.values(diagram.elements);
    const wasmComponents = this.extractWasmComponents(elements);
    ...
    const interfaceMap = new Map<
      string,
      { exporters: string[]; importers: string[] }
    >();
    ...
    interfaceMap.forEach((deps, interfaceName) => {
      if (deps.exporters.length > 0 && deps.importers.length > 0) {
        ...
        yOffset +=
          Math.max(deps.exporters.length, deps.importers.length) * 100 + 150;
      }
    });
    ...
    elements.forEach((element) => {
      ...
      if (elementType === "wasm-component") {
        ...
        const component: WasmComponentData = {
          ...
          size: {
            width: node.bounds?.width || 200,
            height: node.bounds?.height || 150,
          },
        };
    ...
    if (interfaceData) {
      if (Array.isArray(interfaceData)) {
        ...
        return interfaceData.map((iface: any) => ({
          ...
          interface_type:
            iface.interface_type === "export" ? "export" : "import",
          functions:
            iface.functions?.map((func: any) => ({
              name: func.name,
              parameters:
                func.params?.map((p: any) => ({
                  name: p.name,
                  type: p.param_type,
                })) || [],
