...
export class ViewSwitcher {
  ...
  private createViewSwitcher(): HTMLElement {
    ...
    indicator.innerHTML = `
            ...
            <span class="indicator-mode">${this.getViewModeLabel(
              this.currentMode,
            )}</span>
        `;
    ...
    this.viewModes.forEach((mode) => {
      ...
      button.className = `view-mode-btn ${
        mode.id === this.currentMode ? "active" : ""
      }`;
  ...
  private switchMode(modeId: string): void {
    ...
    console.log(
      `ViewSwitcher: Switching from ${this.currentMode} to ${modeId}`,
    );
    ...
    this.container.querySelectorAll(".view-mode-btn").forEach((btn) => {
      const mode = this.viewModes.find(
        (m) => btn.querySelector(".view-mode-label")?.textContent === m.label,
      );
