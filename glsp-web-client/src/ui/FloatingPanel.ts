...
export abstract class FloatingPanel {
  ...
  private createElement(): HTMLElement {
                ...
                <div class="floating-panel-controls">
                    ${
                      this.config.collapsible
                        ? '<button class="panel-btn collapse-btn" title="Collapse">−</button>'
                        : ""
                    }
                    ${
                      this.config.closable
                        ? '<button class="panel-btn close-btn" title="Minimize to Header">×</button>'
                        : ""
                    }
                </div>
            </div>
    ...
    this.headerElement = panel.querySelector(
      ".floating-panel-header",
    ) as HTMLElement;
    this.contentElement = panel.querySelector(
  ...
  private applyInitialStyles(): void {
    ...
    // Controls styles
    const controlsElement = this.element.querySelector(
  ...
  private handleResizing(e: MouseEvent): void {
    ...
    const newWidth = Math.max(
      ...
      Math.min(this.config.maxWidth, currentWidth + deltaX),
    );
    const newHeight = Math.max(
      this.config.minHeight,
      Math.min(this.config.maxHeight, currentHeight + deltaY),
    );
  ...
  public minimizeToHeader(): void {
    console.log(
      "FloatingPanel: minimizeToHeader called for",
      this.config.title,
    );
    this.hide();
    if (this.events.onMinimizeToHeader) {
      ...
      console.warn(
        "FloatingPanel: onMinimizeToHeader callback not set for",
        this.config.title,
      );
    }
  }
