...
export class ViewModeManager {
  ...
  private viewModeListeners: Array<
    (mode: string, previousMode?: string) => void
  > = [];
  ...
  public async switchViewMode(targetMode: string): Promise<boolean> {
    ...
    const targetViewMode = this.viewModes.find(
      (mode) => mode.id === targetMode,
    );
    if (!targetViewMode) {
    ...
    if (!targetViewMode.compatibleDiagramTypes.includes(diagramType)) {
      console.warn(
        `ViewModeManager: View mode ${targetMode} not compatible with diagram type ${diagramType}`,
      );
      return false;
    }
    ...
    if (!transformer) {
      console.warn(
        `ViewModeManager: No transformer registered for diagram type: ${diagramType}`,
      );
      return false;
    }
    ...
    if (
      ...
      console.warn(
        `ViewModeManager: Cannot transform from ${this.currentViewMode} to ${targetMode}`,
      );
      return false;
    }
    ...
    try {
      ...
      if (result.transformedElements) {
        console.log(
          `ViewModeManager: Applying ${result.transformedElements.length} transformed elements`,
        );
        ...
        result.transformedElements.forEach((element) => {
          (transformedDiagram.elements as Record<string, any>)[element.id] =
            element;
        });
      ...
      console.log(
        `ViewModeManager: Successfully switched from ${previousMode} to ${targetMode}`,
      );
      return true;
    } catch (error) {
   ...
   */
  public addViewModeListener(
    listener: (mode: string, previousMode?: string) => void,
  ): void {
    ...
    if (diagramType) {
      ...
      if (availableModes.length > 0) {
        ...
        if (!availableModes.find((mode) => mode.id === this.currentViewMode)) {
          ...
          const previousMode = this.currentViewMode;
          console.log(
            `ViewModeManager: Switching to compatible view mode: ${newMode}`,
          );
   ...
   */
  private async performViewModeTransition(
    renderCallback: () => void,
  ): Promise<void> {
