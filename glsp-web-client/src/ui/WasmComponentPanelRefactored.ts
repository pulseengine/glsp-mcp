...
export class WasmComponentPanel extends FloatingPanel {
  ...
  private setupComponentEventHandlers(): void {
    // Upload button
    const uploadBtn = this.contentElement.querySelector(
    ...
    // Refresh button
    const refreshBtn = this.contentElement.querySelector(
      ".refresh-components-btn",
    ) as HTMLButtonElement;
    ...
    // WIT Analysis button
    const witAnalyzeBtn = this.contentElement.querySelector(
      ".analyze-wit-btn",
    ) as HTMLButtonElement;
    ...
    const retryBtn = this.contentElement.querySelector(
      ".retry-btn",
    ) as HTMLButtonElement;
  ...
  private styleButtons(): void {
    ...
    // Toolbar styles
    const toolbar = this.contentElement.querySelector(
      ".wasm-panel-toolbar",
    ) as HTMLElement;
    if (toolbar) {
  ...
  public async loadComponents(): Promise<void> {
    ...
    try {
      ...
      const timeoutPromise = new Promise<never>((_, reject) => {
        ...
        const timeoutTextElement = this.contentElement.querySelector(
          ".timeout-text",
        ) as HTMLElement;
        const timeoutProgressElement = this.contentElement.querySelector(
          ".timeout-progress",
        ) as HTMLElement;
        ...
        countdownId = window.setInterval(() => {
          ...
          if (timeoutProgressElement) {
            const progress =
              ((timeoutDuration / 1000 - remainingTime) /
  ...
  private showLoadingState(): void {
    const loadingState = this.contentElement.querySelector(
      ".loading-state",
    ) as HTMLElement;
    const emptyState = this.contentElement.querySelector(
      ".empty-state",
    ) as HTMLElement;
    const errorState = this.contentElement.querySelector(
      ".error-state",
    ) as HTMLElement;
    const componentsList = this.contentElement.querySelector(
      ".wasm-components-list",
    ) as HTMLElement;
    ...
    const spinner = loadingState.querySelector(
      ".loading-spinner",
    ) as HTMLElement;
    if (spinner) {
    ...
    const timeoutIndicator = loadingState.querySelector(
    ...
    if (timeoutIndicator) {
      ...
      const progress = timeoutIndicator.querySelector(
        ".timeout-progress",
      ) as HTMLElement;
      if (progress) {
  ...
  private showErrorState(message: string): void {
    ...
    const errorMessageElement = errorState.querySelector(
      ".error-message",
    ) as HTMLElement;
  ...
  private createComponentElement(component: WasmComponentInfo): HTMLElement {
    ...
    element.innerHTML = `
            ...
            <div class="component-meta">
                <span class="interface-count">${
                  component.interfaces
                } interfaces</span>
                ${
                  (component as any).lastSeen
  ...
  private updateComponentCount(count: number): void {
    const countElement = this.contentElement.querySelector(
      ".component-count",
    ) as HTMLElement;
    if (countElement) {
  ...
  public updateClientSideComponents(
    components: import("../wasm/WasmComponentManager.js").WasmComponent[],
  ): void {
    const transpiledComponentsList = this.contentElement.querySelector(
      ".transpiled-components-list",
    ) as HTMLElement;
    const transpiledCountElement = this.contentElement.querySelector(
      ".transpiled-component-count",
    ) as HTMLElement;

    // Check if elements exist before proceeding
    if (!transpiledComponentsList) {
      console.warn(
        "WasmComponentPanel: transpiled-components-list element not found",
      );
      return;
    }

    // Clear existing
    const existingComponents = transpiledComponentsList.querySelectorAll(
      ".client-component-item",
    );
    existingComponents.forEach((item) => item.remove());

    if (components.length === 0) {
      const emptyState = transpiledComponentsList.querySelector(
        ".empty-state",
      ) as HTMLElement;
  ...
  private createClientComponentElement(
    ...
    element.innerHTML = `
            <div class="component-header">
                <div class="component-name">${
                  (component as any).metadata?.name || "Unknown"
                }</div>
                <div class="component-status loaded">🔧</div>
            </div>
            <div class="component-description">Client-side transpiled component</div>
            <div class="component-meta">
                <span class="size-info">${this.formatFileSize(
                  (component as any).metadata?.size || 0,
                )}</span>
  ...
  public updateDiagramComponents(
    components: Array<{
      elementId: string;
      name: string;
    ...
    const listElement = this.contentElement.querySelector(
  ...
  private createDiagramComponentElement(component: {
    ...
    const statusClass = component.isTranspiled
      ? "transpiled"
      : component.isLoaded
        ? "loaded"
        : "unloaded";
  ...
  private setupDiagramComponentEvents(
    element: HTMLElement,
    component: {
      elementId: string;
      name: string;
    ...
    loadBtn?.addEventListener("click", () =>
      this.handleLoadComponent(component.elementId),
    );

    // Unload button
    const unloadBtn = element.querySelector(".unload-btn") as HTMLButtonElement;
    unloadBtn?.addEventListener("click", () =>
      this.handleUnloadComponent(component.elementId),
    );

    // Transpile button
    const transpileBtn = element.querySelector(
      ".transpile-btn",
    ) as HTMLButtonElement;
    transpileBtn?.addEventListener("click", () =>
      this.handleTranspileComponent(component.elementId),
    );

    // Release button
    const releaseBtn = element.querySelector(
      ".release-btn",
    ) as HTMLButtonElement;
    releaseBtn?.addEventListener("click", () =>
      this.handleReleaseComponent(component.elementId),
    );
  }
  ...
  public updateTranspiledComponents(
    components: Array<{
      name: string;
      interfaces: string[];
      source: string;
  ...
  private createTranspiledComponentElement(component: {
    ...
    element.innerHTML = `
            ...
            <div class="component-interfaces">
                ${component.interfaces
                  .map((i) => `<span class="interface-tag">${i}</span>`)
                  .join("")}
            </div>
  ...
  private createTestHarnessElement(harness: {
    ...
    element.innerHTML = `
            ...
            <div class="harness-methods">
                ${harness.methods
                  .map((m) => `<span class="method-tag">${m}()</span>`)
                  .join("")}
            </div>
        `;
  ...
  private async analyzeWitInterfaces(): Promise<void> {
    const analysisContent = this.contentElement.querySelector(
      ".wit-analysis-content",
    ) as HTMLElement;
    ...
    try {
      ...
      const retryBtn = analysisContent.querySelector(
        ".retry-wit-btn",
      ) as HTMLButtonElement;
  ...
  private async getWitInterfacesOverview(): Promise<{
    interfaces?: unknown[];
  }> {
  ...
  private async getWitTypesCatalog(): Promise<{
  ...
  private async getWitDependenciesGraph(): Promise<{
    dependencies?: Array<{ name: string; version?: string }>;
  }> {
    try {
      const result = await this.mcpClient.readResource(
        "wasm://wit/dependencies",
      );
      return JSON.parse(result.text || "{}");
    } catch (error) {
  ...
  private displayWitAnalysisResults(
    overview: {
      summary?: {
        imports?: number;
        exports?: number;
    ...
    typesData: { types?: Array<{ name: string; kind: string }> },
    dependenciesData: {
      dependencies?: Array<{ name: string; version?: string }>;
    },
  ): void {
                ...
                <div class="wit-summary-cards">
                    <div class="summary-card">
                        <div class="card-value">${
                          (summary as any).totalInterfaces || interfaces.length
                        }</div>
    ...
    const detailedBtn = analysisContent.querySelector(
      ".detailed-analysis-btn",
    ) as HTMLButtonElement;
    detailedBtn?.addEventListener("click", () =>
      this.showDetailedWitAnalysis(),
    );
  }
  ...
  private async showDetailedWitAnalysis(): Promise<void> {
    ...
    try {
      const componentsData = await this.mcpClient.readResource(
        "wasm://components/list",
      );
      ...
      alert(
        "Detailed WIT analysis would be shown here. Check console for component data.",
      );
    } catch (error) {
