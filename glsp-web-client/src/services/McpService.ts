...
export class McpService {
  ...
  async createEdge(
    ...
    return this.callTool("create_edge", {
      diagramId,
      edgeType,
      sourceId,
      targetId,
  ...
  async listDiagrams(): Promise<DiagramListResponse> {
    ...
    if (resource && resource.contents && resource.contents[0]?.text) {
  ...
  async readResource(uri: string): Promise<McpResourceResponse> {
    ...
    if (
      wrappedResult.contents &&
      Array.isArray(wrappedResult.contents) &&
      wrappedResult.contents.length > 0
    ) {
      // MCP server is returning wrapped format
      return wrappedResult;
    }

    // Convert direct ResourceContent format to response format
    return { contents: [result as McpResourceContent] };
  }

  async getDiagramModel(
    diagramId: string,
  ): Promise<import("../model/diagram.js").DiagramModel | undefined> {
    const resource = await this.readResource(`diagram://model/${diagramId}`);
    console.log(
      "McpService: Diagram model resource for",
      diagramId,
      ":",
      resource,
    );
    console.log(
      "McpService: Resource has text?",
      !!(resource?.contents && resource.contents[0]?.text),
    );
    console.log(
      "McpService: Resource keys:",
      resource ? Object.keys(resource) : "resource is null",
    );
    if (resource && resource.contents && resource.contents[0]?.text) {
      try {
        const parsed = JSON.parse(resource.contents[0].text);
        ...
        console.error("McpService: Raw text was:", resource.contents[0].text);
        return undefined;
      }
    }
    console.log(
      "McpService: No valid resource.text found, returning undefined",
    );
    return undefined;
  }
  ...
  private broadcastExecutionProgress(progressData: any): void {
    ...
    if (window.interactionManager) {
      try {
        (window.interactionManager as any).handleExecutionProgress?.(
          progressData,
        );
      } catch (error) {
        console.error(
          "Error notifying InteractionManager of execution progress:",
          error,
        );
      }
    }
    ...
    if (window.componentLibrary) {
      try {
        (window.componentLibrary as any).handleExecutionProgress?.(
          progressData,
        );
      } catch (error) {
        console.error(
          "Error notifying ComponentLibrary of execution progress:",
          error,
        );
      }
    }
    ...
    window.dispatchEvent(
      new CustomEvent("wasm-execution-progress", {
        detail: progressData,
      }),
    );
  }
  ...
  private broadcastExecutionResult(resultData: any): void {
    ...
    if (window.interactionManager) {
      try {
        ...
        console.error(
          "Error notifying InteractionManager of execution result:",
          error,
        );
      }
    }
    ...
    // Emit custom event for other listeners
    window.dispatchEvent(
      new CustomEvent("wasm-execution-result", {
        detail: resultData,
      }),
    );
  }
  ...
  private broadcastComponentStatus(statusData: any): void {
    ...
    if (window.componentLibrary) {
      try {
        ...
        console.error(
          "Error notifying ComponentLibrary of component status:",
          error,
        );
      }
    }
    ...
    // Emit custom event for other listeners
    window.dispatchEvent(
      new CustomEvent("wasm-component-status", {
        detail: statusData,
      }),
    );
  }
  ...
  public addConnectionHealthListener(
    listener: import("../mcp/client.js").ConnectionHealthListener,
  ): void {
    this.mcpClient.addConnectionHealthListener(listener);
  }

  public removeConnectionHealthListener(
    listener: import("../mcp/client.js").ConnectionHealthListener,
  ): void {
