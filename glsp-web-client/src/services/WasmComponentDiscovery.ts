...
// Note: WasmInterface and SecurityAnalysis imports removed as they're not used
...
export class WasmComponentDiscovery {
    ...
    try {
      ...
      for (const comp of components) {
        try {
          ...
          const interfaceResource = await this.mcpClient.readResource(
            `wasm://component/${comp.name}/interfaces`,
          );
          const interfaceData = JSON.parse(
            interfaceResource.text || '{"interfaces": []}',
          );
          ...
          console.warn(
            `Failed to fetch interface data for ${comp.name}:`,
            interfaceError,
          );
      ...
      console.log(
        `Discovered ${components.length} WASM components with detailed interface data`,
      );
    } catch (error) {
  ...
  private async checkForChanges(): Promise<void> {
    try {
      ...
      const changesResource = await this.mcpClient.readResource(
        "wasm://changes/recent",
      );
      const changes: WasmComponentChange[] = JSON.parse(
        changesResource.text || "[]",
      );
  ...
  private async refreshComponent(componentName: string): Promise<void> {
    try {
      ...
      if (!response.isError) {
        const componentData: WasmComponent = JSON.parse(
