...
export class DiagramService {
  ...
  private initializeStreaming(): void {
    ...
    console.log(
      "DiagramService: Streaming and notification listeners initialized",
    );
  }

  private handleDiagramChangedNotification(
    notification: import("../mcp/client.js").McpNotification,
  ): void {
    console.log(
      "DiagramService: Received diagram-changed notification:",
      notification,
    );
    try {
      ...
      if (params.diagramId === this.currentDiagramId) {
        ...
        console.log(
          `DiagramService: Reloading diagram ${params.diagramId} due to ${params.changeType} notification`,
        );
  ...
  private handleDiagramDeletedNotification(
    notification: import("../mcp/client.js").McpNotification,
  ): void {
    console.log(
      "DiagramService: Received diagram-deleted notification:",
      notification,
    );
    try {
      ...
      if (params.diagramId === this.currentDiagramId) {
        ...
        console.log(
          `DiagramService: Current diagram ${params.diagramId} was deleted`,
        );
  ...
  private handleValidationCompleteNotification(
    notification: import("../mcp/client.js").McpNotification,
  ): void {
    console.log(
      "DiagramService: Received validation-complete notification:",
      notification,
    );
    try {
      const params = notification.params as {
      ...
      console.log(
        `DiagramService: Validation completed for diagram ${params.diagramId}`,
      );
    } catch (error) {
  ...
  private handleDiagramUpdate(data: unknown): void {
    ...
    try {
      ...
      if (
        updateData.diagramId &&
        this.currentDiagramId === updateData.diagramId
      ) {
  ...
  private handleComponentStatusUpdate(data: unknown): void {
    ...
    try {
      ...
      if (
        statusData.diagramId &&
        statusData.diagramId === this.currentDiagramId
      ) {
        ...
        console.log(
          `DiagramService: Component ${statusData.componentId} status changed to ${statusData.status}`,
        );

        // Update status manager if it's an error
        if (statusData.status === "error" && statusData.message) {
          statusManager.setComponentError(
            statusData.componentId,
            statusData.message,
          );
        }
      }
  ...
  private handleValidationResult(data: unknown): void {
    ...
    try {
      ...
      if (validationData.diagramId === this.currentDiagramId) {
        ...
        const errorCount = validationData.issues.filter(
          (issue) => issue.severity === "error",
        ).length;
        const warningCount = validationData.issues.filter(
          (issue) => issue.severity === "warning",
        ).length;

        if (validationData.status === "completed") {
          if (errorCount > 0) {
            statusManager.setValidationStatus(
              "error",
        ...
        console.log(
          `DiagramService: Validation ${validationData.status} - ${errorCount} errors, ${warningCount} warnings`,
        );
      }
    } catch (error) {
  ...
  public async loadDiagram(
    ...
    try {
      ...
      if (diagram) {
        console.log(
          "DiagramService: About to update diagram state and set current diagram",
        );
        ...
        console.log(
          "DiagramService: Setting current diagram to:",
          diagramId,
          diagramName,
        );
        ...
        console.log(
          `DiagramService: Successfully loaded diagram: ${
            diagram.diagramType || "unknown-type"
          }`,
        );
        ...
        console.log(
          "DiagramService: Diagram load completed successfully, returning diagram",
        );
        ...
        console.warn(
          "DiagramService: getDiagramModel returned null/undefined for:",
          diagramId,
        );
      ...
      if (this.currentDiagramId === diagramId) {
        console.warn(
          `DiagramService: Diagram ${diagramId} no longer exists on server, clearing local reference`,
        );
  ...
  public async createSampleDiagram(): Promise<DiagramModel | undefined> {
    try {
      const result = await this.mcpService.createDiagram(
        "workflow",
        "Sample Workflow",
      );
  ...
  private async addSampleNodes(diagramId: string): Promise<void> {
    try {
      ...
      const startMatch =
        startResult.content?.[0]?.text?.match(/ID: ([a-f0-9-]+)/);
      ...
      const taskMatch =
        taskResult.content?.[0]?.text?.match(/ID: ([a-f0-9-]+)/);
      ...
      const gatewayMatch =
        gatewayResult.content?.[0]?.text?.match(/ID: ([a-f0-9-]+)/);
  ...
  public async createNode(
    ...
    try {
      statusManager.setDiagramSyncStatus("saving");
      await this.mcpService.createNode(
        diagramId,
        nodeType,
        position,
        label,
  ...
  public async deleteDiagram(diagramId: string): Promise<boolean> {
    try {
      ...
      if (
        ...
        (result.content &&
          result.content.some(
            (c: { text?: string }) => c.text && c.text.includes("Unknown tool"),
          ))
      ) {
        console.error(
          "DiagramService: Server returned error for delete:",
          result,
        );
        return false;
      }
  ...
  public onDiagramOperationSuccess(operationType: string): void {
    console.log(
      `DiagramService: ${operationType} operation completed successfully`,
    );
  ...
  public async updateSelectedElementPositions(
    diagramId: string,
    selectedElements: ElementWithBounds[],
  ): Promise<void> {
    try {
      console.log(
        "DiagramService: Updating positions for selected elements:",
      ...
      for (const element of selectedElements) {
        if (element.bounds && element.id) {
          ...
          console.log(
            `DiagramService: Updated position for element ${element.id} to (${element.bounds.x}, ${element.bounds.y})`,
          );
        }
      }
      ...
      statusManager.setDiagramSaved();
      console.log(
        "DiagramService: All element positions updated and saved to server",
      );
    } catch (error) {
      console.error(
        "DiagramService: Failed to update element positions:",
        error,
      );
      statusManager.setDiagramSyncStatus(
        "error",
  ...
  public async notifyDiagramOpened(diagramId: string): Promise<void> {
    try {
      ...
      console.log(
        `DiagramService: Sent diagram-opened notification for ${diagramId}`,
      );
    } catch (error) {
  ...
  public async notifyDiagramModified(
    diagramId: string,
    ...
    try {
      ...
      console.log(
        `DiagramService: Sent diagram-modified notification for ${diagramId} (${modificationType})`,
      );
    } catch (error) {
  ...
  public async notifyElementSelected(
    diagramId: string,
    elementIds: string[],
  ): Promise<void> {
    try {
      ...
      console.log(
        `DiagramService: Sent elements-selected notification for ${elementIds.length} elements`,
      );
    } catch (error) {
  ...
  public async notifyValidationRequested(diagramId: string): Promise<void> {
    try {
      ...
      console.log(
        `DiagramService: Sent validation-requested notification for ${diagramId}`,
      );
    } catch (error) {
