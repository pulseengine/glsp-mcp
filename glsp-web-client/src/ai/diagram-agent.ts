...
Respond with JSON format containing 'analysis' and 'toolCalls' array.`;
  ...
  async createDiagramFromDescription(
    ...
    try {
      ...
      const enhancedPrompt = `Create a ${
        request.diagramType || "workflow"
      } diagram for: "${request.description}"
...
Respond with valid JSON only, no additional text.`;
      ...
      try {
        ...
        steps.push(
          `📋 Parsed ${parsedResponse.toolCalls?.length || 0} tool calls`,
        );
      } catch (parseError) {
  ...
  private async executeToolCalls(
    ...
    for (const toolCall of toolCalls) {
      try {
        steps.push(
          `🔧 Executing: ${toolCall.tool} with ${JSON.stringify(
            toolCall.arguments,
          ).substring(0, 100)}...`,
        );
    ...
    if (
      nodeIds.length > 1 &&
      !toolCalls.some((tc) => tc.tool === "create_edge")
    ) {
  ...
  private async createSimpleDiagramFallback(
    request: DiagramRequest,
    steps: string[],
    errors: string[],
  ): Promise<AgentResponse> {
...
Provide specific recommendations for improving the diagram layout, readability, and flow.`;
  ...
  async analyzeDiagram(
    ...
    try {
      ...
      // Get diagram model
      const modelResource = await this.mcpClient.readResource(
        `diagram://model/${diagramId}`,
      );
      const diagramData = JSON.parse(modelResource.text || "{}");

      // Get validation results
      const validationResource = await this.mcpClient.readResource(
        `diagram://validation/${diagramId}`,
      );
